...
public class AmericanFootballActivity extends AppCompatActivity {
    ...
    private final String AMERICAN_FOOTBALL = "American Football";
    ...
    DatabaseReference americanFootball;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        teamANameHeading = (TextView) findViewById(R.id.teamAName_american_football);
        teamBNameHeading = (TextView) findViewById(R.id.teamBName_american_football);
        teamANameHeading.setText("Team A");
        teamBNameHeading.setText("Team B");
        handler = new Handler();
        gameName = AMERICAN_FOOTBALL;
        americanFootball = FirebaseDatabase.getInstance().getReference();
    ...
    public void undoScore(View view) {
        if (scoreA > 0 && Arr.get(total) == 1) {
            ...
            Arr.remove(total);
            total--;
        } else if (scoreA > 0 && Arr.get(total) == 6) {
            scoreA -= 6;
            Arr.remove(total);
            total--;
            ...
            Arr.remove(total);
            total--;
        } else if (scoreB > 0 && Arr.get(total) == 16) {
            scoreB -= 6;
            Arr.remove(total);
            total--;
        }
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        switch (item.getItemId()) {
            ...
            case R.id.action_history:
                ...
                intent.putExtra("GAME_ID", AMERICAN_FOOTBALL);
    ...
    public void pushingDataToFirebase() {
        ...
        String gameName = AMERICAN_FOOTBALL;
        ...
        int image = R.drawable.american_football_image;

        String id = americanFootball.push().getKey();
        AddScores addScores = new AddScores(image, id, teamAScore, teamBScore, d, gameName, aName, bName, comment);
        americanFootball.child(MainActivity.USER_ID).child(gameName).child(id).setValue(addScores);
