...
public class RugbyActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        teamANameHeading = (TextView) findViewById(R.id.teamANameRugby);
        teamBNameHeading = (TextView) findViewById(R.id.teamBNameRugby);
        teamANameHeading.setText("Team A");
        teamBNameHeading.setText("Team B");
        handler = new Handler();
        gameName = RUGBY;
        rugby = FirebaseDatabase.getInstance().getReference();
    ...
    public void undoScore(View view) {
        if (scoreA > 0 && Arr.get(total) == 5) {scoreA -= 5;Arr.remove(total);total--;}
        ...
        else if (scoreB > 0 && Arr.get(total) == 15) {
            scoreB -= 5;
            Arr.remove(total);
            total--;
    ...
    public void pushingDataToFirebase() {
        ...
        int image = R.drawable.rugby_image;

        String id = rugby.push().getKey();
        AddScores addScores = new AddScores(image, id, teamAScore, teamBScore, d, gameName, aName, bName, comment);
        rugby.child(MainActivity.USER_ID).child(gameName).child(id).setValue(addScores);
