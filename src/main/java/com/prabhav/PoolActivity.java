...
public class PoolActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        whoWho.setText("");
        teamANameHeading = (TextView)findViewById(R.id.teamANamePool);
        teamBNameHeading = (TextView)findViewById(R.id.teamBNamePool);
        teamANameHeading.setText("Team A");
        teamBNameHeading.setText("Team B");
        gameName = POOL;
        pool = FirebaseDatabase.getInstance().getReference();
    }
    ...
    private void updateResults() {
        ...
        else if(scoreA==7&&scoreB==0)
        {
            whoWon.setText(teamANameHeading.getText().toString()+" Won!!");
        }
        else if(scoreB==7&&scoreA==0)
        {
            whoWon.setText(teamBNameHeading.getText().toString()+" Won!!");
        }
        else
    ...
    public void pushingDataToFirebase(){
        ...
        int image =R.drawable.pool_image;

        String id = pool.push().getKey();
        AddScores addScores = new AddScores(image,id,teamAScore,teamBScore,d,gameName,aName,bName,comment);
        pool.child(MainActivity.USER_ID).child(gameName).child(id).setValue(addScores);
